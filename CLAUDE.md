...
boot.py                     — WiFi connect + WebREPL start (runs on every boot)
main.py                     — HTTP server loop (runs after boot.py)
wifi_config_template.py     — credential template (copy → wifi_config.py)
```
...
| `boot.py` | Runs on boot: WiFi connect + WebREPL start | `boot.py` |
| `main.py` | Runs after boot: starts HTTP server loop | `main.py` |
| `wifi_config_template.py` | Credential template (checked in; copy to `wifi_config.py`) | n/a (host only) |
